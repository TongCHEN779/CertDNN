...
#        print('time elapsed: ', end - start)
    ...
    def _maximize_gurobi(cls, f, certificate, n, name=''):
        ...
        m.Params.OutputFlag = 1 #enables solver output
        m.Params.Method = 2 #barrier method
        m.Params.Crossover = 0 #disables crossover option
    ...
    def _alpha_beta_multi_layers_sparse(cls, len_g, start_indices, deg, weights):
        ...
        def variables_connected_to(v, len_g, start_indices, weights):
            ...
            if len(start_indices) == 2:
                final_var = len_g-1
#                final_var = start_indices[1]+np.array(weights[0]).shape[0]-1
            else:
...
#                print(np.array(weights[0]).shape)
                if abs(weights[0][var_right - start_indices[1], v - start_indices[0]]) > 1e-8: #and var_right - start_indices[1] <= np.array(weights[0]).shape[0] - 1:
#                    print(var_right)
                    variables.add(var_right)
                    variables = variables.union(variables_connected_to(var_right, len_g, start_indices[1:], weights[1:]))
#                elif var_right - start_indices[1] > np.array(weights[0]).shape[0] - 1:
#                    variables.add(var_right)
#                    variables = variables.union(variables_connected_to(var_right, len_g, start_indices[1:], weights[1:]))
            return variables
        x_list = [(np.array([]), np.array([]))]
...
#            print('var_right:{}'.format(var_right))
...
#            print('connected_vars:{}'.format(connected_vars))
...
class FullyConnected:
    def __init__(
            self, weights, biases=None, typ="global", lb=None, ub=None, lower=None, upper=None):
        self._weights, self._biases = weights, biases
        self.layer_sizes = np.array([w.shape[1] for w in self._weights])
...
#        print(self.vars_layer)
        self.variables = np.array(
                sp.symbols('x_0:{}'.format(self.layer_sizes.sum())))
#        print(self.variables)
        self.lb, self.ub = lb, ub
#        print(self._weights[1][0])
#        print(self.weights[1][0])
#        print(weights[1][0])
        
        if typ == "local":
            idx_0 = np.where((upper - lower == 0) & (upper == 0))
            idx_1 = np.where((upper - lower == 0) & (upper == 1))
#            self._weights[1][0][idx_0] = 0
            for i in idx_0:
                self.vars_layer[1][i] = 0
#                self.variables[self.layer_sizes[1] + i] = 0
            for i in idx_1:
                self.vars_layer[1][i] = 1
#                self.variables[self.layer_sizes[1] + i] = 1
...
#                print('monomial:{}'.format(monomial))
    ...
    def krivine_constr(self, p=1, typ="global", lower=None, upper=None):
        r"""
...
#            if typ == "global":
#                var_constr = self.variables[var_idx:]
##                var_constr = np.concatenate([self.variables[var_idx:] / 2, (self.variables[var_idx:] + 1) / 2])
#            elif typ == "local":
##                var_constr = self.variables[var_idx:]
#                
#                var_constr = np.multiply(self.variables[var_idx:], (upper - lower)) + upper * (upper - lower == 0) # for ReLU
##                var_constr[0] = self.variables[2]
##                var_constr[1] = self.variables[2]-1
#                idx_0 = np.where(var_constr == 0)
#                idx_1 = np.where(var_constr == 1)
#                var_constr = np.delete(var_constr, np.concatenate([idx_0[0], idx_1[0]]))
#                
##                for i in idx_0[0]:
##                    var_constr = np.concatenate([var_constr, np.array([self.variables[var_idx+i] - 1e-3, 1e-3 - self.variables[var_idx+i]])])
##                for i in idx_1[0]:
##                    var_constr = np.concatenate([var_constr, np.array([self.variables[var_idx+i] - (1 - 1e-3), (1 + 1e-3) - self.variables[var_idx+i]])])
##                a = 1/2
##                for i in idx_0[0]:
##                    var_constr = np.concatenate([var_constr, np.array([self.variables[var_idx+i] + a, a - self.variables[var_idx+i]])])
##                for i in idx_1[0]:
##                    var_constr = np.concatenate([var_constr, np.array([self.variables[var_idx+i] - 1 + a, 1 + a - self.variables[var_idx+i]])])
##                var_constr = ((self.variables[var_idx:] - lower) / (upper - lower) + upper * (upper - lower == 0))[0] # for ELU
#        else:
#            raise NotImplementedError
    ...
    def sparse_krivine_constr(self, p=1):  # Why do we need this?
        ...
        pattern = onelayer_sparsity_pattern(self._weights[0])
        g = [None] * len(pattern)
...
def norm_gradients_local(f, d, x00, eps=0.1, n=10000, p=1):
    x = 2 * eps * torch.rand(n, d) + torch.FloatTensor(np.array(x00, dtype=float) - eps)
    x.requires_grad = True
    f(x).sum().backward()
    norms = torch.norm(x.grad.data, p=p, dim=1)
    return norms
...
def d_relu(x):
    r"""Derivative of ReLU activation function"""
    return (x > 0) * 1. + (x <= 0.) * 0
